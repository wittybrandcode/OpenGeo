...
**إصدار الوثيقة:** 1.3<br>
**تاريخ الإنشاء:** 22 أغسطس 2026  
**آخر مراجعة:** 1 سبتمبر 2026<br>
**الحالة:** T1 منفذة برمجياً وتنتظر اعتماد AE بصرياً؛ لا تبدأ T2 قبل اختبار 4K منخفض التكبير وعبور 180°<br>
...
**خط الأساس الآلي الحالي:** `npm test` — ‏144/144 ناجحة في 1 سبتمبر 2026 على commit `15cd52b`<br>
**الانحدار السابق للتنفيذ:** BASE-001 مغلق؛ ثُبت أن `revision = 0` Host sentinel وليس عضواً في الساعة الدائرية<br>
**بوابة T1 الآلية:** ‏144/144 ناجحة؛ FI-05 خضراء، و9/9 أعطال لاحقة ما زالت expected-red، بلا Harness errors
...
| T1 | عقد الهوية: Download مقابل Placement | 9 | 15 | 🟨 | T0 | 16 تنزيلاً تغذي 72 موضعاً في fixture ‏4K/Zoom 2؛ بانتظار AE |
...
| **الإجمالي** |  | **98** | **100** | **8% معتمد / 23% برمجي** |  |  |
...
| المراحل الموزونة المكتملة ✅ | 1/9 |
| المراحل المنفذة برمجياً ✅ + 🟨 | 2/9 |
| المهام التنفيذية المكتملة | 26/98 |
| شروط Definition of Done المكتملة | 0/18 |
| النقاط المعتمدة | 8/100 |
| اختبارات الخط الأساسي والمضافة | 144/144 ناجحة على `15cd52b` |
| اختبارات fault injection | FI-05 محلولة؛ 9/9 expected-red متبقية؛ 0 Harness errors |
| اختبارات AE الجديدة المعتمدة | 0 |
| الموانع | اعتماد AE بصري لـT1 مطلوب قبل فتح T2 |
...
- [ ] Composition ‏3840×2160 عند Zoom 2–4: لا فراغ أسود على جانبي العالم، وFinalize يطابق Preview.
- [ ] تحريك/مسار يعبر 180°: لا قفزة إلى العالم الخطأ ولا تنزيلات مكررة ظاهرة.
- [ ] Composition ‏1920×1080 عادية: لا تغير في الموضع أو المقياس مقارنة بالسلوك السليم السابق.
- [ ] Path Preview مع Keyframes متباعدة: كل موضع عالمي ظاهر في الزمن الصحيح.
...
الخطوة التالية هي **اعتماد T1 داخل After Effects** وفق الاختبارات الأربعة المسجلة أعلاه. بعد نجاحها تتحول T1 من 🟨 إلى ✅ وتبدأ **T2 فقط**: بوابة اكتمال التنزيل fail-closed لـLive Sync وPath Preview وFinalize، من دون تعديل Worker أو cache أو retry policy قبل مراحلها المحددة.
