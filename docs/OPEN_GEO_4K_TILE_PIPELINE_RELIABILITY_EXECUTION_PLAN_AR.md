...
**إصدار الوثيقة:** 1.6<br>
**تاريخ الإنشاء:** 22 أغسطس 2026  
**آخر مراجعة:** 1 سبتمبر 2026<br>
**الحالة:** اختبارات T4 الأساسية معتمدة داخل AE؛ بقي اعتماد انحدار Add Key → Finalize بعد إصلاح سباق الكتابة/المسح<br>
...
**خط الأساس الآلي الحالي:** `npm test` — ‏150/150 ناجحة في 1 سبتمبر 2026 على commit `d8ff4cc`<br>
**الانحدار السابق للتنفيذ:** BASE-001 مغلق؛ ثُبت أن `revision = 0` Host sentinel وليس عضواً في الساعة الدائرية<br>
**بوابة T4 الآلية:** ‏150/150 ناجحة؛ FI-01…FI-05 وFI-07 وFI-08 خضراء، و3/3 أعطال لمراحل لاحقة ما زالت expected-red، بلا Harness errors
...
| T4 | سلامة MegaTile والـWorker والكاش | 13 | 20 | 🟨 | T0, T2 | PNG/manifest معتمدة؛ بقي انحدار Add Key → Finalize |
...
| **الإجمالي** |  | **98** | **100** | **41% معتمد / 61% برمجي** |  |  |
...
| المراحل الموزونة المكتملة ✅ | 3/9 |
| المراحل المنفذة برمجياً ✅ + 🟨 | 4/9 |
| المهام التنفيذية المكتملة | 50/98 |
| شروط Definition of Done المكتملة | 0/18 |
| النقاط المعتمدة | 41/100 |
| اختبارات الخط الأساسي والمضافة | 150/150 ناجحة على `d8ff4cc` |
| اختبارات fault injection | FI-01…FI-05 وFI-07 وFI-08 محلولة؛ 3/3 expected-red متبقية؛ 0 Harness errors |
| اختبارات AE الجديدة المعتمدة | 9؛ منها AE-T4-01…05 |
| الموانع | إعادة اختبار AE-T4-06 بعد إصلاح سباق Add Key/Finalize مطلوبة قبل فتح T5 |
...
- [ ] **AE-T4-06 — Add Key → Finalize barrier:** ضع مفتاحاً أولاً، انتقل إلى زمن وموقع/Zoom مختلفين، ثم اضغط Add Key وFinalize مباشرة من دون انتظار Path Preview؛ يجب أن ينتظر Finalize اكتمال كتابة المفتاح وأن يشمل الناتج النهائي الحركة بين المفتاحين. لا يقبل إضافة مفتاح جديد بعد بدء مسح Finalize.
...
- **قرار الإغلاق:** تبقى T4 في 🟨 حتى نجاح AE-T4-06 مرة واحدة بعد إعادة تحميل الإضافة؛ عند النجاح تتحول مباشرة إلى ✅ وتفتح T5.
