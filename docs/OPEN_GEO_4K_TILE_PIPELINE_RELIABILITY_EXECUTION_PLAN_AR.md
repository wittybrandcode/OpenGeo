...
**إصدار الوثيقة:** 1.2<br>
**تاريخ الإنشاء:** 22 أغسطس 2026  
**آخر مراجعة:** 1 سبتمبر 2026<br>
**الحالة:** T0 مكتملة ومثبتة بـ10/10 حالات فشل متوقعة؛ جاهزة لبدء T1 دون أي تعديل إنتاجي سابق لأوانه<br>
...
**خط الأساس الآلي الحالي:** `npm test` — ‏140/140 ناجحة في 1 سبتمبر 2026 على commit `375bb94`<br>
**الانحدار السابق للتنفيذ:** BASE-001 مغلق؛ ثُبت أن `revision = 0` Host sentinel وليس عضواً في الساعة الدائرية<br>
**خط الأساس المعتمد لـT1:** ‏140/140 ناجحة؛ `verify:tile-pipeline:red-baseline` يثبت 10/10 حالات حمراء متوقعة بلا Harness errors
...
| **الإجمالي** |  | **98** | **100** | **8% معتمد / 8% برمجي** |  |  |
...
| المراحل المنفذة برمجياً ✅ + 🟨 | 1/9 |
| المهام التنفيذية المكتملة | 17/98 |
| شروط Definition of Done المكتملة | 0/18 |
| النقاط المعتمدة | 8/100 |
| اختبارات الخط الأساسي | 140/140 ناجحة على `375bb94` |
| اختبارات regression الجديدة | 10/10 expected-red موثقة؛ 0 Harness errors |
| اختبارات AE الجديدة المعتمدة | 0 |
| الموانع | لا يوجد؛ T1 متاحة |
...
الخطوة التالية هي **T1 فقط**: فصل `downloadKey` عن `placementKey` من دون تغيير retry أو Worker أو Finalize. يبدأ التنفيذ بتعريف العقدين canonical، ثم تعديل `TilePlanner` ليعيد طلبات تنزيل فريدة مع جميع مواضع العرض، وتحديث المستهلكين والاختبارات حتى تتحول FI-05 إلى أخضر مع بقاء FI-01…04 وFI-06…10 حمراء للأسباب المسجلة.
