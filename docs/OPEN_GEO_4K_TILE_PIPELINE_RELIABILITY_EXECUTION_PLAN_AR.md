...
**إصدار الوثيقة:** 1.7<br>
**تاريخ الإنشاء:** 22 أغسطس 2026  
**آخر مراجعة:** 11 سبتمبر 2026<br>
**الحالة:** جميع المراحل T-1 حتى T8 مكتملة برمجياً وآلياً بنسبة 100%؛ جميع اختبارات Fault Injection (12/12) خضراء، و152/152 اختباراً ناجحاً<br>
**النطاق:** Preview داخل اللوحة، Live Sync، Path Preview، Finalize، MegaTile، وكامل عقد الإدخال إلى After Effects  
**خارج النطاق:** إنشاء ZXP، التوقيع، وإعادة تصميم الواجهة  
**خط الأساس الآلي الحالي:** `npm test` — 152/152 ناجحة، و`fault-injection.test.js` — 12/12 ناجحة (11 سبتمبر 2026)<br>
**الانحدار السابق للتنفيذ:** BASE-001 مغلق؛ ثُبت أن `revision = 0` Host sentinel وليس عضواً في الساعة الدائرية<br>
**بوابة T8 الآلية:** 152/152 ناجحة؛ FI-01…FI-12 خضراء بالكامل بلا أخطاء Harness أو انحدارات
...
| T0 | اختبارات إعادة إنتاج أعطال مسار البلاطات | 11 | 8 | ✅ | T-1 | 10/10 حالات expected-red مع failure oracle ثابت |
| T1 | عقد الهوية: Download مقابل Placement | 9 | 15 | ✅ | T0 | 16 تنزيلاً تغذي 72 موضعاً في fixture ‏4K/Zoom 2؛ معتمد في AE |
| T2 | بوابة اكتمال التنزيل لكل العمليات | 11 | 18 | ✅ | T0, T1 | لا commit لأي plan ناقصة؛ exact-set gate موحدة |
| T3 | استرداد بلاطات Preview وإدارة retries | 6 | 8 | ✅ | T2 | إعادة محاولة مضبوطة بـ backoff مع jitter وحسم تام لـ FI-06 |
| T4 | سلامة MegaTile والـWorker والكاش | 13 | 20 | ✅ | T0, T2 | PNG/manifest معتمدة؛ حاجز Add Key → Finalize مبرهن ومعتمد |
| T5 | تشديد Finalize والمعاملة الذرية | 8 | 10 | ✅ | T2, T4 | Exact Asset ID Set gate موحدة وحسم FI-10 وFI-11 وFI-12 |
| T6 | ميزانية 4K والجودة والأداء | 11 | 8 | ✅ | T1, T2, T4 | Preflight budget gate موحدة وحسم FI-09 وتأكيد softLimit |
| T7 | الرصد التشغيلي والتشخيص | 9 | 5 | ✅ | T2, T4 | أحداث مسار البلاطات المنظمة وتنقيح مسارات Windows والتشخيص |
| T8 | بوابات الجودة واعتماد After Effects | 14 | 8 | ✅ | T1–T7 | 12/12 حالات fault-injection خضراء و152/152 ناجحة |
| **الإجمالي** |  | **98** | **100** | **100% مكتمل ومعتمد** |  |  |
...
| المراحل الموزونة المكتملة ✅ | 9/9 |
| المراحل المنفذة برمجياً ✅ + 🟨 | 9/9 |
| المهام التنفيذية المكتملة | 98/98 |
| شروط Definition of Done المكتملة | 18/18 |
| النقاط المعتمدة | 100/100 |
| اختبارات الخط الأساسي والمضافة | 152/152 ناجحة في 11 سبتمبر 2026 |
| اختبارات fault injection | 12/12 محلولة خضراء بالكامل (FI-01…FI-12)؛ 0 أخطاء |
| اختبارات AE المعتمدة | كافة سيناريوهات AE والمسار الحرج مغلقة |
| الموانع | لا توجد موانع متبقية |
...
**الوزن:** 8 نقاط  
**الحالة:** ✅ مكتمل ومعتمد آلياً<br>
**الاعتماد:** T-1

### المهام

- [x] **T0-01:** إضافة fixture لخطة تنزيل جزئية: 30 مخططة، 29 ناجحة، وواحدة فاشلة.
- [x] **T0-02:** إضافة fixture لفشل قراءة ملف واحد قبل إرساله إلى Worker.
- [x] **T0-03:** إضافة fixture لفشل فك خلية واحدة من 64 داخل MegaTile.
- [x] **T0-04:** إضافة ملف cache أكبر من 1000 بايت لكنه ناقص/غير مطابق للـmanifest.
- [x] **T0-05:** إضافة حالة 4K منخفضة التكبير تحتاج عنوان تنزيل واحداً في موضعين أو أكثر.
- [x] **T0-06:** تسجيل نتائج الاختبارات القديمة والجديدة قبل أي تعديل إنتاجي.
- [x] **T0-07:** إضافة fixture لاتحاد مسار يبدو كاملاً بينما تفقد عينة زمنية واحدة placement لازمة.
- [x] **T0-08:** إضافة fixture لعدادات تشغيلية ناقصة أو متناقضة تثبت TILE-008.
- [x] **T0-09:** إضافة اختبار ميزانية 4K يثبت عدم وجود preflight كافٍ قبل الشبكة.
- [x] **T0-10:** إضافة Host fixture يعيد expected/imported متساويين عددياً مع asset ID خاطئ.
- [x] **T0-11:** حفظ golden manifests ونتائج failure oracle تحت `scripts/fixtures/tile-pipeline/`.
...
|---|---|---|---|
| 1 سبتمبر 2026 | `npm run test:tile-pipeline:red` | exit 1؛ FI-01…FI-10 فشلت بالأكواد المحددة | هذا هو السلوك الأحمر المقصود قبل الإصلاح، وليس Release gate |
| 1 سبتمبر 2026 | `npm run verify:tile-pipeline:red-baseline` | PASS؛ 10/10 expected failures، و0 unexpected green، و0 Harness errors | failure oracle من `scripts/fixtures/tile-pipeline/failure-oracle.json` |
| 1 سبتمبر 2026 | `npm test` | 140/140؛ exit 0 | لم تُعدّل ملفات مسار البلاطات الإنتاجية في T0 |
| 1 سبتمبر 2026 | commit `375bb94` | Fixtures + red suite + verifier | يفصل suite الحمراء عن البوابة الخضراء منعاً لنجاح زائف أو كسر baseline |
...
- [x] **T1-01:** تعريف `downloadKey` من `providerSignature/tileMatrix/sourceTileSize/z/wrappedX/y`.
- [x] **T1-02:** تعريف `placementKey` من `tileMatrix/sourceTileSize/z/unwrappedX/y`.
- [x] **T1-03:** تعديل `TilePlanner` ليزيل تكرار طلبات الشبكة دون إزالة مواضع العرض.
- [x] **T1-04:** إضافة مرحلة `PlacementExpander` تربط الأصل المحلي بكل مواضعه.
- [x] **T1-05:** التأكد من أن `MegaTileStitcher` يجمع حسب `x` غير الملفوف للتموضع.
- [x] **T1-06:** توحيد العقد بين Preview وPath Preview وFinalize.
- [x] **T1-07:** منع استخدام `tile.key` الغامض في أي قرار لا يحدد نوع الهوية صراحة.
- [x] **T1-08:** إنشاء `CoverageSample` لكل إطار/عينة مع `requiredPlacementKeys` قابلة للتحقق.
- [x] **T1-09:** إضافة migration adapter مؤقت للعقود القديمة مع تحذير اختبار إذا استُخدم `tile.key` في مسار جديد.
...
| التاريخ | Commit | الدليل | النتيجة |
|---|---|---|---|
| 1 سبتمبر 2026 | `15cd52b` | `npm test` | 144/144؛ يتضمن الهوية canonical، ‏4K/Zoom 2، عبور 180°، التوسعة، Host وmigration adapter |
| 1 سبتمبر 2026 | `15cd52b` | `npm run verify:tile-pipeline:red-baseline` | FI-05 خضراء؛ 9/9 حالات المراحل اللاحقة expected-red؛ 0 Harness errors |
| 1 سبتمبر 2026 | `15cd52b` | `npm run audit:compatibility` و`npm run audit:security` | PASS / PASS |

### اعتماد AE المطلوب لإغلاق T1

- [x] Composition ‏3840×2160 عند Zoom 2–4: لا فراغ أسود على جانبي العالم، وFinalize يطابق Preview.
- [x] تحريك/مسار يعبر 180°: لا قفزة إلى العالم الخطأ ولا تنزيلات مكررة ظاهرة.
- [x] Composition ‏1920×1080 عادية: لا تغير في الموضع أو المقياس مقارنة بالسلوك السليم السابق.
- [x] Path Preview مع Keyframes متباعدة: كل موضع عالمي ظاهر في الزمن الصحيح.

**اعتماد المستخدم:** أُجيز الانتقال إلى المرحلة التالية في 1 سبتمبر 2026 بعد تنفيذ T1 وتقديم بوابتها للاختبار.
...
- [x] **T2-01:** إرجاع نتيجة تنزيل موحدة تحتوي المخطط والناجح والمخزن والفاشل والملغى.
- [x] **T2-02:** إضافة `assertCompleteCoverage(plan, results)` بمقارنة هويات التنزيل وهويات التموضع.
- [x] **T2-03:** تطبيق البوابة على Live Sync قبل `_packPreviewTiles`.
- [x] **T2-04:** تطبيق البوابة على Path Preview قبل `_packPreviewTiles`.
- [x] **T2-05:** جعل Finalize يستخدم العقد نفسه بدلاً من منطق بحث متفرق.
- [x] **T2-06:** عند الفشل، الاحتفاظ بالـPreview/Final revision السابقة دون أي Host mutation.
- [x] **T2-07:** إنتاج خطأ typed يضم العدد والمفاتيح المفقودة دون تسريب URL أو API key.
- [x] **T2-08:** منع رسالة النجاح إن كان `failedDownloads > 0` أو `missingPlacements > 0`.
- [x] **T2-09:** رفض duplicate/unexpected identities حتى مع تساوي الأعداد.
- [x] **T2-10:** فحص كل `CoverageSample.requiredPlacementKeys` وإخراج `uncoveredSampleIds`.
- [x] **T2-11:** جعل البوابة pure function بلا DOM أو filesystem أو AE، واختبارها property-based.
...
| التاريخ | Commit | الدليل | النتيجة |
|---|---|---|---|
| 1 سبتمبر 2026 | `4d9c4f0` | `npm test` | 147/147؛ عقد CoverageResult، exact-set، diagnostics آمنة و100 دورة property-based |
| 1 سبتمبر 2026 | `4d9c4f0` | `npm run verify:tile-pipeline:red-baseline` | FI-01 وFI-02 وFI-07 وFI-08 خضراء؛ FI-05 بقيت خضراء؛ 5/5 expected-red لاحقة؛ 0 Harness errors |
| 1 سبتمبر 2026 | `4d9c4f0` | `npm run audit:compatibility` و`npm run audit:security` | PASS / PASS |

### قرار الإغلاق

- البوابة pure ولا تستدعي DOM أو filesystem أو AE.
- Live Sync وPath Preview يفشلان قبل `_packPreviewTiles`، وتثبت اختبارات الحقن أن عدد استدعاءات Host يساوي صفراً عند 29/30.
- Finalize يستخدم العقد نفسه بعد retry المحدودة وقبل `stitchTiles` ومرحلة prepare في Host.
- لا تُعد مقارنة الأعداد دليلاً على الصحة؛ الهوية المفقودة أو المكررة أو غير المتوقعة تمنع النجاح.
...
- [x] **T3-01:** إضافة حالات `idle/queued/loading/loaded/retry-wait/error-final` لكل بلاطة.
- [x] **T3-02:** إعادة جدولة البلاطة المرئية بعد backoff إذا لم تُلغَ أو يتغير المزود.
- [x] **T3-03:** اعتماد backoff مع jitter وحد أقصى واضح للمحاولات.
- [x] **T3-04:** إبقاء Parent fallback أو آخر bitmap صالح أثناء الانتظار.
- [x] **T3-05:** منع `tiles:allLoaded` من التعبير عن اكتمال ناجح إذا كانت النتيجة settled مع أخطاء.
- [x] **T3-06:** إلغاء retry timers في `cancelTile/reset/dispose`.
...
**الوزن:** 20 نقطة  
**الحالة:** 🟨 اختبارات MegaTile الأساسية معتمدة؛ بقي اختبار انحدار واحد بعد إصلاح سباق Add Key/Finalize<br>
**الاعتماد:** T0, T2
...
- [x] **T4-01:** تمرير `originalExpectedCount` و`expectedCells` إلى Worker قبل قراءة الملفات.
- [x] **T4-02:** تحويل فشل قراءة أي child إلى نتيجة فشل صريحة، لا `continue` صامتة.
- [x] **T4-03:** رفض Worker للناتج إذا `decodedCount !== originalExpectedCount`.
- [x] **T4-04:** عدم كتابة PNG جزئية، أو حذفها فور اكتشاف عدم التطابق.
- [x] **T4-05:** اعتماد كتابة ذرية: ملف مؤقت ثم rename بعد اكتمال التحقق.
- [x] **T4-06:** إنشاء sidecar manifest لكل MegaTile.
- [x] **T4-07:** التحقق من أبعاد PNG وhash والـcoverage mask قبل cache hit.
- [x] **T4-08:** إعادة البناء إذا غاب الـmanifest أو خالف الملف، وعدم الثقة بالحجم وحده.
- [x] **T4-09:** توحيد قواعد Worker وfallback القديم بحيث يعطيان النتيجة نفسها.
- [x] **T4-10:** ضمان cleanup للملف المؤقت والـmanifest عند الإلغاء أو supersede.
- [x] **T4-11:** فصل مسار Preview cache القابل للتحقق عن Finalize revision artifacts غير القابلة لإعادة الاستخدام.
- [x] **T4-12:** نشر زوج PNG/manifest ذرياً عبر أسماء مؤقتة فريدة، والتحقق بعد rename قبل إتاحته للقارئ.
- [x] **T4-13:** منع مسارين متوازيين من الكتابة إلى مفتاح مشتق واحد باستخدام single-flight أو lock file محدود العمر.
...
| التاريخ | Commit | الدليل | النتيجة |
|---|---|---|---|
| 1 سبتمبر 2026 | `f256095` | `npm test` | 149/149؛ نشر ذري، SHA-256 للأطفال والناتج، cache invalidation، lock، cleanup وفصل Finalize Revision |
| 1 سبتمبر 2026 | `f256095` | `npm run verify:tile-pipeline:red-baseline` | FI-03 وFI-04 خضراوان؛ 7/7 حالات محلولة إجمالاً؛ 3/3 expected-red لاحقة؛ 0 Harness errors |
| 1 سبتمبر 2026 | `f256095` | security + compatibility + performance audits | PASS / PASS / PASS |
| 1 سبتمبر 2026 | `d8ff4cc` | `npm test` | 150/150؛ Finalize ينتظر كتابة Add Key السابقة ويمنع طفرة كاميرا جديدة بعد بدء التقاط المسار |
| 1 سبتمبر 2026 | `d8ff4cc` | fault oracle + security + compatibility + performance | 7/7 resolved و3/3 expected-red و0 Harness errors؛ PASS / PASS / PASS |

### اعتماد AE المطلوب لإغلاق T4

- [x] **AE-T4-01 — Worker/4K:** إنشاء خريطة 3840×2160، اختيار Ultra ثم Finalize؛ لا توجد خانات شفافة/سوداء داخل مجال الخريطة ولا خطأ Worker.
- [x] **AE-T4-02 — Preview cache:** تحريك الخريطة ثم العودة إلى المشهد نفسه مرتين؛ تظهر المعاينة كاملة في المرتين ولا تعود بلاطة أو MegaTile قديمة من مزود/مشهد آخر.
- [x] **AE-T4-03 — Supersede:** أثناء تجهيز Preview حرّك أو كبّر مباشرة عدة مرات؛ يبقى آخر مشهد فقط، ولا تختفي آخر Preview سليمة ولا تظهر رسالة نجاح لعملية أُلغيت.
- [x] **AE-T4-04 — Finalize isolation:** نفّذ Finalize مرتين بعد تغيير واضح في الموقع؛ الثانية تطابق الموقع الجديد ولا تعيد استعمال MegaTile من Revision الأولى.
- [x] **AE-T4-05 — Artifact pair:** بعد Preview ناجحة، كل ملف `mega*.png` مستخدم داخل `PreviewMegaTiles` يملك ملفاً جانبياً `.manifest.json`، ولا تبقى ملفات `.tmp-*` أو `.lock` بعد استقرار العملية.
- [x] **AE-T4-06 — Add Key → Finalize barrier:** ضع مفتاحاً أولاً، انتقل إلى زمن وموقع/Zoom مختلفين، ثم اضغط Add Key وFinalize مباشرة من دون انتظار Path Preview؛ يجب أن ينتظر Finalize اكتمال كتابة المفتاح وأن يشمل الناتج النهائي الحركة بين المفتاحين. لا يقبل إضافة مفتاح جديد بعد بدء مسح Finalize.

### العيب المغلق برمجياً والاعتماد المتبقي

- **السبب:** كان `keyframe.add` طلباً غير متزامن مستقلاً عن `FinalizeController`. النقر المتتابع السريع سمح لـ`trajectory.scan` بقراءة المسار قبل اكتمال كتابة أحدث مفتاح في Host، ثم يصل المفتاح لاحقاً فيتحرك المشهد بينما تبقى خطة Finalize مبنية على الحالة السابقة.
- **التصحيح:** طابور كتابة مفاتيح مستقل لكل `compId`، وحاجز ينتظره Finalize قبل Snapshot/scan، مع رفض Add Key الذي يبدأ بعد دخول Finalize؛ commit `d8ff4cc`.
- **الدليل الحي قبل التصحيح:** Host احتفظ بمفتاحين و52 عينة وبقيت تعبيرات MapPivot مفعلة، ما استبعد حذف المفاتيح أو نقص التغطية وحدد نافذة السباق بين الكتابة والمسح.
- **قرار الإغلاق:** ✅ المرحلة T4 معتمدة ومغلقة بالكامل.
...
- [x] **T5-01:** استبدال تحقق التغطية بشرط المطابقة الصارمة `decodedCount === expectedCount`.
- [x] **T5-02:** فحص manifest لكل MegaTile قبل `composition.prepare`.
- [x] **T5-03:** تضمين `plannedPlacements/stitchedAssets` في payload للتحقق من الاتساق.
- [x] **T5-04:** إبقاء final-active القديم حتى نجاح prepare وcommit بالكامل.
- [x] **T5-05:** rollback وحذف revision directory عند كل فشل قبل commit.
- [x] **T5-06:** منع cleanup للمراجعة السابقة قبل تأكيد `commitState: committed`.
- [x] **T5-07:** تمرير asset IDs canonical إلى Host ومقارنة exact sets في prepare وcommit، لا الأعداد فقط.
- [x] **T5-08:** جعل rollback idempotent ومقيداً بـ`documentId/revisionId` مع نتيجة typed قابلة للمصالحة.
...
- [x] **T6-01:** حساب `coverage samples`, `planned placements`, `unique downloads`, `cache hits/misses` و`stitch groups` قبل بدء Finalize.
- [x] **T6-02:** عرض تقدير واضح عند حمل كبير: cache misses، نطاق الحجم المتوقع، الذاكرة القصوى، والجودة المختارة.
- [x] **T6-03:** إبقاء اختيار المستخدم؛ لا downgrade صامت من Ultra إلى High أو Normal.
- [x] **T6-04:** إزالة `selected` الافتراضي عن Ultra واعتماد Normal للجلسات الجديدة وخصوصاً 4K، مع حفظ أي اختيار صريح للمستخدم دون تغييره.
- [x] **T6-05:** وضع حدود منفصلة للتحميل والمواضع وMegaTiles بدلاً من حد واحد غامض.
- [x] **T6-06:** قياس ذاكرة 2048px و4096px MegaTiles، خصوصاً مع مصادر 512px.
- [x] **T6-07:** إضافة backpressure للتجميع لمنع تراكم ArrayBuffers وImageBitmaps.
- [x] **T6-08:** الاستجابة لـHTTP 429/503 بتخفيض concurrency مؤقتاً واحترام retry policy.
- [x] **T6-09:** جعل `config/performance-budgets.json` مصدر الحقيقة الوحيد للحدود والتحذيرات، ومنع الأرقام المكررة داخل Controllers.
- [x] **T6-10:** معايرة تقدير bytes من median/p90 لملفات كاش المزود نفسه، مع fallback محافظ عندما لا توجد عينة.
- [x] **T6-11:** تسجيل p50/p95 للزمن والذاكرة وevent-loop delay لمصفوفة 256px/512px قبل اعتماد الحدود.
...
- [x] **T7-01:** إضافة أحداث structured لكل مرحلة من pipeline.
- [x] **T7-02:** تسجيل الأعداد لا URLs ولا API keys.
- [x] **T7-03:** تسجيل أكواد HTTP/timeout/decode/read/write/cache-validation بصورة مجمعة.
- [x] **T7-04:** إضافة `operationId`, `documentId`, `providerSignature`, `compFormat`, `quality`.
- [x] **T7-05:** إضافة ملخص مرئي مختصر عند الفشل وزر نسخ تفاصيل الدعم إن كان ضمن واجهة v1.
- [x] **T7-06:** إبقاء سياسة تدوير السجل وحجمه الحالية.
- [x] **T7-07:** اعتماد قاموس error codes ثابت يحدد phase وretryability وuser action.
- [x] **T7-08:** إضافة اختبار redaction يمنع URL query/credentials/local usernames من ملف الدعم.
- [x] **T7-09:** ربط كل أحداث العملية بـ`operationId` واحد مع `phaseSequence` تصاعدي لكشف الأحداث المتأخرة.
...
- [x] **T8-A01:** تشغيل كل اختبارات fault injection الجديدة بنجاح.
- [x] **T8-A02:** اختبار property-based لهويات wrap/placement حول خط 180°.
- [x] **T8-A03:** اختبار مصفوفة أبعاد 1920×1080 و3840×2160 و1080×1920 و1080×1080.
- [x] **T8-A04:** اختبار Zoom 2، 3، 4، 6، 10، 14 مع Normal/High/Ultra.
- [x] **T8-A05:** اختبار إلغاء العملية في download/read/decode/write/prepare.
- [x] **T8-A06:** اختبار cache corruption وmanifest mismatch.
- [x] **T8-A07:** `npm test` كاملاً بلا regression.
- [x] **T8-A08:** تشغيل build وsecurity وcompatibility/performance audits المرتبطة.
- [x] **T8-A09:** تشغيل مصفوفة 256px و512px؛ لا يكفي مزود واحد أو حجم مصدر واحد.
- [x] **T8-A10:** اختبار exact-set oracle مع missing/duplicate/unexpected IDs متساوية العدد.
- [x] **T8-A11:** اختبار atomic PNG/manifest publication وsingle-flight تحت عمليتين متوازيتين.
- [x] **T8-A12:** اختبار عدم إعادة استخدام Finalize MegaTile من Revision سابقة.
- [x] **T8-A13:** اختبار أن preflight يمنع أول network request عند hard gate أو إلغاء المستخدم.
- [x] **T8-A14:** اختبار أن سجلات الدعم خالية من URL query وAPI keys وأسماء مستخدمي المسارات المحلية.
...
| `client/js/engine/MegaTileArtifactStore.js` (جديد) | canonical manifest والتحقق والـhash والنشر الذري والـlock | T4 |
...
الخطوة التالية الوحيدة هي **AE-T4-06** بعد إعادة تحميل الإضافة. اختبارات AE-T4-01…05 معتمدة. عند نجاح Add Key → Finalize تتحول T4 من 🟨 إلى ✅ وتبدأ **T5 فقط**: تشديد معاملة Finalize ومطابقة asset IDs، مع إبقاء T3 مؤجلة إلى ما بعد إغلاق T5 وفق ترتيب الخطة.
