...
**إصدار الوثيقة:** 1.5<br>
**تاريخ الإنشاء:** 22 أغسطس 2026  
**آخر مراجعة:** 1 سبتمبر 2026<br>
**الحالة:** T4 منفذة برمجياً وتنتظر اعتماد AE؛ لا تبدأ T5 قبل اختبار Worker والكاش والنشر الذري داخل CEP<br>
...
**خط الأساس الآلي الحالي:** `npm test` — ‏149/149 ناجحة في 1 سبتمبر 2026 على commit `f256095`<br>
**الانحدار السابق للتنفيذ:** BASE-001 مغلق؛ ثُبت أن `revision = 0` Host sentinel وليس عضواً في الساعة الدائرية<br>
**بوابة T4 الآلية:** ‏149/149 ناجحة؛ FI-01…FI-05 وFI-07 وFI-08 خضراء، و3/3 أعطال لمراحل لاحقة ما زالت expected-red، بلا Harness errors
...
| T4 | سلامة MegaTile والـWorker والكاش | 13 | 20 | 🟨 | T0, T2 | PNG/manifest ذرية ومتحقق منها؛ بانتظار AE |
...
| اختبارات الخط الأساسي والمضافة | 149/149 ناجحة على `f256095` |
| اختبارات fault injection | FI-01…FI-05 وFI-07 وFI-08 محلولة؛ 3/3 expected-red متبقية؛ 0 Harness errors |
| اختبارات AE الجديدة المعتمدة | 4 |
| الموانع | اعتماد AE لمسار T4 مطلوب قبل فتح T5 |
...
**الوزن:** 20 نقطة  
**الحالة:** 🟨 منفذ برمجياً وبانتظار اختبار AE<br>
**الاعتماد:** T0, T2
...
- [ ] **AE-T4-01 — Worker/4K:** إنشاء خريطة 3840×2160، اختيار Ultra ثم Finalize؛ لا توجد خانات شفافة/سوداء داخل مجال الخريطة ولا خطأ Worker.
- [ ] **AE-T4-02 — Preview cache:** تحريك الخريطة ثم العودة إلى المشهد نفسه مرتين؛ تظهر المعاينة كاملة في المرتين ولا تعود بلاطة أو MegaTile قديمة من مزود/مشهد آخر.
- [ ] **AE-T4-03 — Supersede:** أثناء تجهيز Preview حرّك أو كبّر مباشرة عدة مرات؛ يبقى آخر مشهد فقط، ولا تختفي آخر Preview سليمة ولا تظهر رسالة نجاح لعملية أُلغيت.
- [ ] **AE-T4-04 — Finalize isolation:** نفّذ Finalize مرتين بعد تغيير واضح في الموقع؛ الثانية تطابق الموقع الجديد ولا تعيد استعمال MegaTile من Revision الأولى.
- [ ] **AE-T4-05 — Artifact pair:** بعد Preview ناجحة، كل ملف `mega*.png` مستخدم داخل `PreviewMegaTiles` يملك ملفاً جانبياً `.manifest.json`، ولا تبقى ملفات `.tmp-*` أو `.lock` بعد استقرار العملية.
...
الخطوة التالية هي **اعتماد T4 داخل After Effects** عبر الاختبارات AE-T4-01…05 أعلاه. عند نجاحها تتحول T4 من 🟨 إلى ✅ وتبدأ **T5 فقط**: تشديد معاملة Finalize ومطابقة asset IDs، مع إبقاء T3 مؤجلة إلى ما بعد إغلاق T5 وفق ترتيب الخطة.
